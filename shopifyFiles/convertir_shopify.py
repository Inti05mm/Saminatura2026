...
def main():
    ...
    ap.add_argument("--variant-meta-out", default="variant_metafields_pending.csv")
    a=ap.parse_args()
    ...
    good=[p for p in products if truthy(p.get("isgood")) and truthy(p.get("is_active")) and not truthy(p.get("is_discontinued"))]

    groups=defaultdict(list)
    for p in good:
        gid=clean(p.get("variant_group_id"))
        key=f"group:{gid}" if gid else f"product:{clean(p.get('id')) or clean(p.get('slug'))}"
        groups[key].append(p)
    ...
    for key, members in groups.items():
        ...
        for i,p in enumerate(members):
            ...
            if i==0:
                ...
                r["isgood (product.metafields.custom.isgood)"]="TRUE"
    ...
    print(f"Productos buenos:          {len(good)}")
