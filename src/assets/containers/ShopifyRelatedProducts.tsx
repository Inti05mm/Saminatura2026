...
export default function ShopifyRelatedProducts({
  category,
  currentProductId,
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        console.error("Error cargando relacionados Shopify:", error);

        setErr(
          error instanceof Error
            ? error.message
            : "Error cargando relacionados"
        );
  ...
  if (err) {
    return (
      <div className="mt-10">
        <div className="w-full px-4 md:px-8">
          <div className="rounded-[28px] border border-gray-200 bg-white p-6 shadow-[0_16px_45px_rgba(17,24,39,0.05)]">
            <p className="font-semibold text-red-700">No se pudieron cargar relacionados</p>
                    ...
                    return (
                      <div key="moreWrap" className="shrink-0 px-2" style={{ width: itemWidth }}>
                        <button
                          type="button"
                          onClick={() =>
                            navigate(`/tienda?category=${encodeURIComponent(category)}#products`)
                          }
