...
type ShopifyProductRow = {
  ...
  // Visibilidad real en el canal Online Store.
  // Un producto puede seguir ACTIVE pero estar oculto.
...
export default function ShopifyAdminProductsManager() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      // Si era un borrador pendiente, al revisarlo:
      // 1) ya lo acabamos de pasar a ACTIVE + isgood=true;
      // 2) ahora lo publicamos en Online Store.
      if (editing.status === "DRAFT") {
        const visibilityResponse =
          await fetch(
            `${SHOPIFY_API_BASE}/shopify/admin/products/${editing.numeric_id ?? editing.id}/visibility`,
            {
              method: "POST",
              headers: {
        ...
        const visibilityData =
          await visibilityResponse.json();

        if (
          !visibilityResponse.ok ||
          visibilityData.ok === false
        ) {
          throw new Error(
            getApiError(
              visibilityData,
              "El producto se revisó, pero no se pudo publicar en Online Store."
            )
          );
        }
      }
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Todos los productos de Shopify en una sola lista. Filtra por activos o borradores. Los borradores se revisan desde el mismo editor y al guardar pasan a activos.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 xl:flex-row xl:items-center">
          ...
          <select
            ...
            className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm xl:w-44"
          >
            <option value="ALL">
              Todos
            </option>
                        ...
                        <td className="whitespace-nowrap px-4 py-3 text-right">
                          {row.status === "DRAFT" ? (
                            <button
                              ...
                              onClick={() =>
                                void reviewDraft(row)
                              }
                              className="rounded-md border border-blue-300 px-3 py-1 text-blue-800 hover:bg-blue-50 disabled:opacity-50"
                            >
                              Marcar revisado
                            </button>
                          ) : (
...
<div className="relative z-[100000] flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl bg-white shadow-lg">            <div className="flex shrink-0 items-center justify-between border-b px-5 py-4">
              <div className="flex items-center gap-3">
                <img
                  src={
                    editing.image_url ||
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {editing.status === "DRAFT"
                      ? "Al guardar se marcará como revisado y pasará a activo. SKU, barcode, coste y stock siguen bloqueados por FireSoft."
                      : "Datos de tienda editables. Los datos FireSoft permanecen bloqueados."}
                  </p>
                </div>
              </div>
